...
<div class="max-w-lg mx-auto bg-white p-6 sm:p-8 rounded-3xl shadow-lg border border-gray-100 text-center mt-6">
    ...
    <div id="sender-view" style="display: none;">
        ...
        <!-- HTML 버튼 자체에 다이렉트 명령을 심었습니다 -->
        <button id="pick-btn" onclick="window.generateMenu()" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-xl shadow-md transition-all text-lg mb-3">
            랜덤 메뉴 뽑기 🎯
        </button>
        ...
        <div id="share-box" style="display: none;" class="mt-6 p-5 bg-green-50 rounded-xl border border-green-200">
            ...
            <div class="flex gap-2">
                <button onclick="window.copyShareLink()" class="flex-1 bg-green-500 hover:bg-green-600 text-white font-bold py-3 rounded-lg shadow transition-all flex items-center justify-center gap-2">
                    <span>🔗 카톡으로 공유</span>
                </button>
                <button onclick="window.resetLunch()" class="px-5 bg-gray-500 hover:bg-gray-600 text-white font-bold py-3 rounded-lg shadow transition-all">
                    🔄 다시
                </button>
            </div>
        </div>
    ...
    <div id="receiver-view" style="display: none;">
        ...
        <div id="gift-box" class="cursor-pointer" onclick="window.openGift()">
        ...
        <div id="result-box" style="display: none;" class="transform scale-90 opacity-0 transition-all duration-500">
            ...
            <button onclick="window.resetLunch()" class="w-full bg-gray-800 hover:bg-gray-900 text-white font-bold py-3 rounded-xl shadow transition-all">
                나도 친구한테 메뉴 뽑아주기 🚀
            </button>
        </div>
    </div>
...
<script>
// 테마가 코드를 지우지 못하도록 브라우저 최상단 객체(window)에 모든 기능을 못 박아 둡니다.
window.lunchMenus = [
...
window.generateMenu = function() {
    ...
    const interval = setInterval(() => {
        const randomItem = window.lunchMenus[Math.floor(Math.random() * window.lunchMenus.length)];
        ...
        if (count >= 15) {
            ...
            const finalItem = window.lunchMenus[Math.floor(Math.random() * window.lunchMenus.length)];
            document.getElementById('roulette-box').innerText = '🤫';
            document.getElementById('roulette-text').innerText = '결과가 상자에 담겼습니다!';
            
            window.lunchGeneratedUrl = window.location.origin + window.location.pathname + '?m=' + finalItem.id;
...
window.copyShareLink = function() {
    navigator.clipboard.writeText(window.lunchGeneratedUrl).then(() => {
...
// 화면이 먹통이 되지 않도록 무조건 페이지 자체를 새로고침 해버립니다.
...
window.openGift = function() {
    ...
    const finalMenu = window.lunchMenus.find(m => m.id === menuId) || window.lunchMenus[0];
...
// 페이지가 켜질 때 화면 세팅
window.initLunchView();

// 깃허브 테마가 화면을 휙휙 넘길 때를 대비한 방어막
document.addEventListener('turbolinks:load', window.initLunchView);
document.addEventListener('pjax:success', window.initLunchView);
</script>
