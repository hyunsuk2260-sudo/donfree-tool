...
(function() {
    ...
    // 버튼들에 직접 클릭 기능을 달아주는 핵심 함수 (에러 해결)
    function initApp() {
        enforceViewMode();
        
        const pickBtn = document.getElementById('pick-btn');
        if(pickBtn) pickBtn.onclick = generateMenu;
        ...
        if(restartBtn) restartBtn.onclick = function() { location.href='/lunch/'; };
    }

    // 테마가 방해하기 전에 브라우저 켜지자마자 실행
