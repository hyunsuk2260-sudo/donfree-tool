...
<div class="max-w-lg mx-auto bg-white p-6 sm:p-8 rounded-3xl shadow-lg border border-gray-100 text-center mt-6" markdown="0">
    ...
    <!-- 1️⃣ [보내는 사람 화면] 기본 노출 (에러 시 하얀 화면 방지) -->
    <div id="view-sender">
...
<!-- 즉시 화면 분기 스크립트 (화면 로딩되자마자 0.01초만에 판별) -->
<script>
(function() {
...
{% raw %}
<script>
// 테마 간섭 방지를 위해 window 객체에 모든 데이터를 묶습니다.
window.lunchData = [
...
window.doPick = function() {
    ...
    var timer = setInterval(function() {
        var rnd = window.lunchData[Math.floor(Math.random() * window.lunchData.length)];
        document.getElementById('r-box').innerText = rnd.img;
        document.getElementById('r-text').innerText = rnd.name;
        c++;
        if(c >= 15) {
            clearInterval(timer);
            var finalItem = window.lunchData[Math.floor(Math.random() * window.lunchData.length)];
            document.getElementById('r-box').innerText = '🤫';
            document.getElementById('r-text').innerText = '결과가 상자에 담겼습니다!';
            window.lunchUrl = window.location.origin + window.location.pathname + '?m=' + finalItem.id;
...
window.doCopy = function() {
    navigator.clipboard.writeText(window.lunchUrl).then(function() {
        alert('링크 복사 완료! 카톡에 붙여넣기 하세요.');
    }).catch(function() {
        alert('복사 실패! 주소창의 주소를 복사해주세요.\n' + window.lunchUrl);
    });
};

// 복잡한 새로고침 로직 대신 화면 자체를 강제로 갓 켠 상태로 되돌립니다.
...
window.doOpen = function() {
    ...
    var menu = window.lunchData.find(function(item) { return item.id === id; }) || window.lunchData[0];
