...
<!-- 깃허브 마크다운 에러 방지용 전체 감싸기 -->
<div id="tarot-app-container">
    <script src="https://cdn.tailwindcss.com"></script>

    <div class="bg-[#1a1a1a] text-white p-6 sm:p-8 rounded-2xl shadow-xl max-w-3xl mx-auto text-center mt-6 border border-gray-800">
        <h2 class="text-2xl sm:text-3xl font-bold mb-2 text-purple-400">오늘의 운명 타로</h2>
        <p class="text-gray-400 mb-8 text-xs sm:text-sm">마음속으로 질문을 떠올린 후, 직감적으로 가장 끌리는 카드를 하나 선택하세요.</p>
        
        <!-- 1. 여러 장의 카드 펼쳐놓기 (선택 영역) -->
        <div id="card-selection" class="flex flex-wrap justify-center gap-3 sm:gap-4 mb-4">
            <!-- 7장의 카드 뒷면 -->
            <div class="w-20 h-32 sm:w-24 sm:h-36 bg-gradient-to-br from-indigo-600 via-purple-700 to-pink-600 rounded-lg border-2 border-yellow-500 shadow-lg flex items-center justify-center cursor-pointer hover:-translate-y-4 transition-transform duration-300" onclick="drawTarot(this)">
        ...
        <div id="result-area" class="hidden mt-4 animate-fade-in">
            ...
            <!-- 다시 뽑기 버튼 -->
            <button onclick="resetTarot()" class="mt-8 px-8 py-3 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-bold rounded-full transition-all shadow-md hover:shadow-lg transform hover:-translate-y-1">다른 질문으로 다시 뽑기 ↻</button>
        </div>
    </div>
    ...
    function drawTarot(clickedElement) {
        // 1. 선택지 화면 숨기기
