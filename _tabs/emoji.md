...
<input type="text" id="searchInput" placeholder="검색 (예: 하트, 맥주, 인사, 바다)..." class="w-full px-4 py-2.5 bg-gray-100 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-pink-500 focus:bg-white transition-all">
</div>
</header>
...
<div id="emojiGrid" class="grid grid-cols-6 sm:grid-cols-8 gap-2 bg-white p-4 rounded-2xl border border-gray-100 shadow-sm"></div>
</main>
...
var matchCat=(window.currentCat==='all'||item.c===window.currentCat);
var matchSearch=(window.searchQuery===''||item.k.includes(window.searchQuery)||item.e.includes(window.searchQuery));
...
return '<button type="button" onclick="window.copyEmojiData(\''+item.e+'\')" class="h-11 sm:h-12 flex items-center justify-center text-2xl sm:text-3xl hover:bg-pink-50 hover:scale-110 active:scale-95 rounded-xl transition-all duration-150 cursor-pointer border border-transparent hover:border-pink-100" title="'+item.k+'">'+item.e+'</button>';
}).join('');
};
