...
<!-- 🖥️ PC 화면 분할 영역 (왼쪽: 툴 / 오른쪽: 뉴스) -->
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
    
    <!-- 👈 왼쪽 구역 (툴 메뉴, 화면의 2/3 차지) -->
    ...
    <!-- 👉 오른쪽 구역 (뉴스 피드, 화면의 1/3 차지) -->
    <div class="lg:col-span-1" markdown="0">
        <div class="bg-white p-4 sm:p-5 rounded-2xl shadow-md border border-gray-200 h-full">
            ...
            <!-- 뉴스가 들어갈 공간 -->
            <ul id="auto-news-list" class="m-0 p-0 list-none">
                <li class="text-gray-500 text-xs py-4 text-center">뉴스를 불러오는 중...</li>
            </ul>
        </div>
    </div>
...
{% raw %}
<script>
// 무한 로딩 방지: 페이지 눈치를 보지 않고 즉시 스크립트를 실행합니다.
(function() {
    ...
    fetch(apiUrl)
        ...
        .then(data => {
            ...
            // 뉴스를 7개 긁어와서 공간을 꽉 채웁니다.
            const articles = data.items.slice(0, 7);
            
            articles.forEach(article => {
                let cleanTitle = article.title.split(' - ')[0];
                const li = document.createElement('li');
                // 뉴스 리스트 사이에 옅은 줄(border)을 넣어 가독성을 높였습니다.
                li.className = 'py-2.5 first:pt-1 border-b border-gray-100 last:border-0';
                li.innerHTML = `
                    <a href="${article.link}" target="_blank" class="group block text-decoration-none">
        ...
        .catch(error => {
            const list = document.getElementById('auto-news-list');
            if(list) list.innerHTML = '<li class="text-xs text-red-500 py-2 text-center">뉴스를 불러오지 못했습니다.<br>새로고침(F5)을 눌러주세요.</li>';
        });
})();
</script>
{% endraw %}
